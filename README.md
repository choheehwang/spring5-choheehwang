...
#### 20201204(금) 작업
- 임베디드 프로그램에서 char를 사용하는 이유: 메모리를 효율적으로 사용하기 위함
- 아두이노 임베디드 프로그램 배울 때 char 사용 예정
...
- 수료하실때쯤 면접일정에 맞춰서, 여러분 프로젝트를 파스타에 배포 하시는 것을 추천
...
- 헤로쿠 Deploy메뉴에서 작업을 했는데, 적용이 안 되시고, 예전내용이 보이시는 경우,
...
- 최종 결과: https://choheehwang.herokuapp.com (반응형 홈페이지 나오게 됩니다.)
...
####20201124(화) 작업
- 외부 스크립트 불러올 때는 위치가 중요하다.
- 실행파일 확장자. .jar(자바archive실행파일),.war(웹archive실행), .apk(안드로이드Package실행),.aab(안드로이드AppBundle실행)
...
- 1. 우선 클래스명은 대문자로 시작합니다.(필수)
- 2. 변수/함수명은 소문자로 시작합니다.(선택적 필수)
- 3. 특히 CSS에서는 클래스명에 -(하이픈)이 가능합니다. 단, j쿼리(자바스크립트)에서는 변수/함수명에 - 이 들어가면 안됩니다.
- CDN(Contents Delivery Network): 콘텐츠를 네트워크로 전달받아서 사용하는 방식.
- 어제 수업시 작동 안 되던 네이티브 자바스크립트 사용예시
- 어제 작동이 안 되었던 이유: 위에서 아래로 순서를 가지고 실행이 됩니다.
...
- 파스타 정상작동이 확인될때까지는 헤로쿠에서 계속 작업예정 입니다.
...
- 오늘 답변메일 못받으신 분은 파스타 질문답변게시판에 체험하기요청 답신메일이 안 온다고 요청부탁드립니다.
...
- pom.xml에서 수정사항이 발생되면, 메이븐 업데이트가 필요합니다.
...
- VT 안되시는 분은 3차 방법으로 수업가능합니다. 염려하실 필요 없습니다.
...
- 실습은 학원PC에서 하지 못하시는 관계로 여러분 집에 있는 PC로 실습을 진행하시게 됩니다.
